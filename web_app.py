...
if not st.session_state['user_info']:
                ...
                # CASE 5: GIỌNG AI VIENEU (LOCAL PC)
                    ...
                    # NHÁNH 2: NẾU CHƯA CÓ KẾT QUẢ -> HIỆN FORM CHỌN GIỌNG & NÚT GỬI
                        ...
                        if 'pending_tts_id' in st.session_state:
                            ...
                            st.info("⏳ AI đang tạo giọng nói... Bạn có thể đợi hoặc đi làm việc khác rồi quay lại xem video ở nút Danh sách video.")
    ...
    if not hide_step_3:
        ...
        if user.get('plan') == 'huynhde':
            c_space, c_outro = st.columns([3, 2])
            with c_outro:
                chk_outro_main = st.checkbox("(Tùy chọn) Kèm website hatbuinho.com ở cuối", value=False, key="chk_outro_main")
