...
if not st.session_state['user_info']:
    ...
    MAX_ITEMS = 3
    total_items = len(history_df) if not history_df.empty else 0
    
    if not history_df.empty:
        status_map = {
            "Pending": "⏳ Đang chờ xử lý",
            "Processing": "⚙️ Đang tạo video...",
            "Done": "✅ Hoàn thành - Bấm xem",
            "Error": "❌ Gặp lỗi",
            "": "❓ Chưa xác định"
        }

        # --- [LOGIC MỚI] GIỚI HẠN HIỂN THỊ (LAZY LOAD) ---
        if 'history_expanded' not in st.session_state:
            st.session_state['history_expanded'] = False
        
        # Nếu chưa bấm mở rộng thì cắt data, ngược lại lấy hết
        if st.session_state['history_expanded']:
            df_display = history_df
        else:
            df_display = history_df.head(MAX_ITEMS)

        # Helper: Giữ trạng thái expander khi bấm nút
        def set_active_order(oid):
            st.session_state['active_order_id'] = oid

        # Vòng lặp hiển thị danh sách (đã cắt gọn)
        for index, row in df_display.iterrows():
            date_str = row.get('NgayTao', '')
            result_link = row.get('LinkKetQua', '')
            raw_status = row.get('TrangThai', 'Pending')
            order_id = row.get('ID', f'Unknown_{index}')
            
            # [NEW] Lấy thông tin để tái sử dụng
            old_audio_link = row.get('LinkGiongNoi', '')
            old_content_script = row.get('NoiDung', '')
            
            # --- [MỚI] TẠO TRÍCH DẪN NGẮN (PREVIEW) ---
            # Lấy 10 từ đầu tiên để hiển thị ra ngoài cho dễ tìm
            try:
                words = str(old_content_script).split()
                if len(words) > 10:
                    script_preview = " ".join(words[:10]) + "..."
                else:
                    script_preview = str(old_content_script)
            except:
                script_preview = ""
            # -------------------------------------------
            ...
            try:
                ...
                clean_date_name = date_str.strftime('%d%m%Y')
            except:
                display_date = str(date_str)
                clean_date_name = "Video"

            # Kiểm tra xem dòng này có đang được active không
            is_expanded = (st.session_state.get('active_order_id') == order_id)

            # [ĐÃ SỬA] Thêm script_preview vào tiêu đề expander
            with st.expander(f"{display_date} | {vn_status} | 📝 {script_preview}", expanded=is_expanded):
                # 1. HIỂN THỊ KẾT QUẢ (NẾU CÓ)
                if result_link and str(result_link).startswith("http"):
                    # [ĐÃ XÓA] Dòng st.success("✅ Video đã hoàn thành!") theo yêu cầu
                    
                    # --- XỬ LÝ LINK TẢI (Fix cho iOS) ---
                    download_link = result_link
                    if "cloudinary.com" in str(result_link) and "/upload/" in str(result_link):
                        download_link = result_link.replace("/upload/", "/upload/fl_attachment/")
                    
                    # --- GIAO DIỆN NÚT BẤM (TONE NÂU) ---
                    col_btn1, col_btn2 = st.columns([1, 1], gap="small")
                    
                    # CSS chung: Bo góc, đổ bóng nhẹ
                    btn_style = "width: 100%; padding: 10px; border-radius: 8px; text-align: center; font-weight: bold; text-decoration: none; display: block; box-shadow: 0 2px 3px rgba(0,0,0,0.1);"
                    
                    with col_btn1:
                        # Nút 1: Xem Video - MÀU NÂU SÁNG (#8D6E63)
                        # Màu nhẹ nhàng, mời gọi bấm vào xem
                        btn_watch_html = f"""
                        <a href="{result_link}" target="_blank" style="{btn_style} background-color: #8D6E63; color: white; border: 1px solid #6D4C41;">
                            ▶️ XEM VIDEO
                        </a>
                        """
                        st.markdown(btn_watch_html, unsafe_allow_html=True)
                        
                    with col_btn2:
                        # Nút 2: Tải File - MÀU NÂU ĐẤT (#5D4037)
                        # Màu đậm hơn một chút để phân biệt
                        btn_dl_html = f"""
                        <a href="{download_link}" target="_self" style="{btn_style} background-color: #5D4037; color: white; border: 1px solid #3E2723;">
                            📥 TẢI VỀ MÁY
                        </a>
                        """
                        st.markdown(btn_dl_html, unsafe_allow_html=True)

                else:
                    if raw_status == "Error": st.error("Đơn này bị lỗi.")
                    else: st.info("Hệ thống đang xử lý...")

                # [NEW] NÚT TẠO LẠI (Giữ màu nâu mặc định của App - #8B4513)
                st.markdown('<div style="margin-top: 5px;"></div>', unsafe_allow_html=True) 
                
                if old_audio_link and str(old_audio_link).startswith("http"):
                    if st.button(f"♻️ Tạo lại bằng Audio này", key=f"recreate_{order_id}", 
                                 disabled=is_out_of_quota, 
                                 use_container_width=True, 
                                 help="Dùng lại audio và nội dung cũ"):
                        if is_out_of_quota:
                            st.error("Bạn đã hết Quota!")
                        else:
                            # ... (Logic tạo lại giữ nguyên như cũ) ...
                            try:
                                with st.spinner("Đang gửi yêu cầu tạo lại..."):
                                    gc = get_gspread_client()
                                    ws = gc.open(DB_SHEET_NAME).worksheet(DB_WORKSHEET)
                                    now_vn = datetime.utcnow() + timedelta(hours=7)
                                    new_order_id = now_vn.strftime("%Y%m%d_%H%M%S")
                                    # Chống trùng ID
                                    try:
                                        existing_ids = ws.col_values(1)
                                        while new_order_id in existing_ids:
                                            now_vn += timedelta(seconds=1)
                                            new_order_id = now_vn.strftime("%Y%m%d_%H%M%S")
                                    except: pass
                                    new_timestamp = now_vn.strftime("%Y-%m-%d %H:%M:%S")

                                    ws.append_row([
                                        new_order_id, new_timestamp, user['email'], "Re-created", 
                                        old_content_script, old_audio_link, "Pending", "", json.dumps(settings)
                                    ])
                                    log_history(new_order_id, user['email'], "", new_timestamp)
                                    update_user_usage(user['row'], user['quota_used'])
                                    st.session_state['user_info']['quota_used'] += 1
                                    
                                    # [MOI] Xóa cache để cập nhật lịch sử ngay lập tức
                                    get_all_orders_cached.clear()
                                    # [MOI] Bật thông báo chờ 5 phút
                                    st.session_state['show_wait_message'] = True
                                    
                                    st.success(f"✅ ĐÃ GỬI LẠI THÀNH CÔNG! Mã: {new_order_id}")
                                    st.rerun()
                            except Exception as e:
                                st.error(f"Lỗi: {e}")
    ...
    if total_items > MAX_ITEMS:
        st.markdown("---")
        col_center = st.columns([1, 2, 1])[1] # Canh giữa
        with col_center:
            if not st.session_state['history_expanded']:
                # Thêm key="btn_load_more" để fix cứng lỗi Duplicate ID
                if st.button(f"🔽 Xem thêm ({total_items - MAX_ITEMS} video cũ hơn)", use_container_width=True, key="btn_load_more"):
                    st.session_state['history_expanded'] = True
                    st.rerun()
            else:
                # Thêm key="btn_collapse"
                if st.button("🔼 Thu gọn danh sách", use_container_width=True, key="btn_collapse"):
                    st.session_state['history_expanded'] = False
                    st.rerun()
    else:
        st.info("Bạn chưa có video nào.")
